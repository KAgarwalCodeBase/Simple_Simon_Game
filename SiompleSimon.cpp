...
int main(void) 
{  
    char another_game = 'Y';             // Records if another game to be played  
  ...
  do  {    // Initialize game    
    ...
    // Inner loop continues as long as sequences are entered correctly    
      ...
      // Now overwrite the digit sequence                 
				...
				for(unsigned int i = 1 ; i <= digits ; ++i)        printf("  ");                  // Output two spaces 
    ...
    // Calculate and output the game score    
		 ...
		 total_digits = digits*(((tries % 3) == 0) ? 3 : tries % 3);    if(digits > 2)      
